...
std::vector<double> UZrxeMomentPartials;

UZrSuperCluster::UZrSuperCluster(int numMax, int nTot, IReactionNetwork& _network,
		std::shared_ptr<xolotlPerf::IHandlerRegistry> registry) :
		UZrCluster(_network, registry,
	...
	type = ReactantType::UZrSuper;

	return;
}

void UZrSuperCluster::resultFrom(ProductionReaction& reaction,
		IReactant& product) {
...
void UZrSuperCluster::resultFrom(ProductionReaction& reaction, double *coef) {

	// Create a new SuperClusterProductionPair
	SuperClusterProductionPair superPair(
			&static_cast<UZrCluster&>(reaction.first),
			&static_cast<UZrCluster&>(reaction.second), &reaction);
...
void UZrSuperCluster::participateIn(ProductionReaction& reaction,
		IReactant& product) {
...
void UZrSuperCluster::participateIn(ProductionReaction& reaction, double *coef) {
	// Look for the other cluster
	auto& otherCluster = static_cast<UZrCluster&>(
			(reaction.first.getId() == id) ? reaction.second : reaction.first);
...
void UZrSuperCluster::participateIn(DissociationReaction& reaction,
		IReactant& disso) {
	// Look for the other cluster
...
void UZrSuperCluster::participateIn(DissociationReaction& reaction,
	...
	SuperClusterDissociationPair superPair(
			&static_cast<UZrCluster&>(reaction.dissociating), &emittedCluster,
			&reaction);
	// Update the coeficients
...
void UZrSuperCluster::emitFrom(DissociationReaction& reaction,
		IReactant& disso) {
	// Cast the reacting clusters
	auto& cluster1 = static_cast<UZrCluster&>(reaction.first);
	auto& cluster2 = static_cast<UZrCluster&>(reaction.second);
...
void UZrSuperCluster::emitFrom(DissociationReaction& reaction, double *coef) {
...
double UZrSuperCluster::getMoment() const {
	return l1;
}

double UZrSuperCluster::getTotalConcentration() const {
...
double UZrSuperCluster::getTotalXenonConcentration() const {
	// Initial declarations
	int index = 0;
	double distance = 0.0, conc = 0.0;
...
double UZrSuperCluster::getDistance(int xe) const {
	if (nTot == 1)
		return 0.0;
	return 2.0 * (double) (xe - numXe) / ((double) nTot - 1.0);
}

void UZrSuperCluster::resetConnectivities() {
	// Clear both sets
	...
	UZrxeMomentPartials.resize(dof, 0.0);

	return;
}

double UZrSuperCluster::getTotalFlux(int i) {
...
double UZrSuperCluster::getDissociationFlux(int xi) {
	// Initial declarations
	double flux = 0.0, value = 0.0;
	UZrCluster *dissociatingCluster = nullptr;
...
double UZrSuperCluster::getEmissionFlux(int xi) {
	// Initial declarations
	double flux = 0.0, value = 0.0;
...
double UZrSuperCluster::getProductionFlux(int xi) {
	// Local declarations
	double flux = 0.0, value = 0.0;
	UZrCluster *firstReactant = nullptr, *secondReactant = nullptr;
...
double UZrSuperCluster::getCombinationFlux(int xi) {
	// Local declarations
	double flux = 0.0, value = 0.0;
	UZrCluster *combiningCluster = nullptr;
...
void UZrSuperCluster::getPartialDerivatives(std::vector<double> & partials,
		int i) const {
	// Reinitialize the moment partial derivatives vector
	std::fill(UZrxeMomentPartials.begin(), UZrxeMomentPartials.end(), 0.0);
...
void UZrSuperCluster::getProductionPartialDerivatives(
	...
	for (auto it = effReactingList.begin(); it != effReactingList.end(); ++it) {
		...
		UZrxeMomentPartials[index] += value
...
void UZrSuperCluster::getCombinationPartialDerivatives(
		std::vector<double> & partials, int xi) const {
	// Initial declarations
	int index = 0;
	...
	for (auto it = effCombiningList.begin(); it != effCombiningList.end();
		...
		UZrxeMomentPartials[index] -= value * ((*it).a001 * l0 + (*it).a101 * l1);
		index = cluster->getMomentId() - 1;
		partials[index] -= value * ((*it).a010 * l0 + (*it).a110 * l1);
		UZrxeMomentPartials[index] -= value * ((*it).a011 * l0 + (*it).a111 * l1);
...
void UZrSuperCluster::getDissociationPartialDerivatives(
		std::vector<double> & partials, int xi) const {
	// Initial declarations
	int index = 0;
	UZrCluster *cluster = nullptr;
	...
	for (auto it = effDissociatingList.begin(); it != effDissociatingList.end();
		...
		UZrxeMomentPartials[index] += value * ((*it).a01);
		index = cluster->getMomentId() - 1;
		partials[index] += value * ((*it).a10);
		UZrxeMomentPartials[index] += value * ((*it).a11);
	}

	return;
}

void UZrSuperCluster::getEmissionPartialDerivatives(
		std::vector<double> & partials, int xi) const {
	// Initial declarations
	int index = 0;
	double value = 0.0;
...
void UZrSuperCluster::getMomentPartialDerivatives(
		std::vector<double> & partials) const {
	// Loop on the size of the vector
	for (int i = 0; i < partials.size(); i++) {
		// Set to the values that were already computed
		partials[i] = UZrxeMomentPartials[i];
	}

	return;
}

std::vector<std::vector<double> > UZrSuperCluster::getProdVector() const {
	// Initial declarations
...
std::vector<std::vector<double> > UZrSuperCluster::getCombVector() const {
	// Initial declarations
	std::vector<std::vector<double> > toReturn;
...
std::vector<std::vector<double> > UZrSuperCluster::getDissoVector() const {
	// Initial declarations
	std::vector<std::vector<double> > toReturn;
...
std::vector<std::vector<double> > UZrSuperCluster::getEmitVector() const {
	// Initial declarations
	std::vector<std::vector<double> > toReturn;
